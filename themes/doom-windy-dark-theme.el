;;; doom-windy-dark-theme.el --- based on Tailwind colors -*- lexical-binding: t; no-byte-compile: t; -*-
;;
...
(defgroup doom-windy-dark-theme nil
    "Options for the `doom-windy-dark' theme."
    :group 'doom-themes)

(defcustom doom-windy-dark-brighter nil
    "If non-nil, more vivid colors will be used."
    :group 'doom-windy-dark-theme
    :type 'boolean)

(defcustom doom-windy-dark-padded-modeline doom-themes-padded-modeline
...
(def-doom-theme doom-windy-dark
    "A dark theme inspired by Tailwind."
    :family 'doom-windy
    ...
    (
        ;; name       default   256       16
        ;; Define theme colors
        (gray-50     '("#f9fafb" "#f7f7f7" "white"))
        (gray-200    '("#e5e7eb" "#e3e3e3" "white"))
        (zinc-700    '("#3f3f46" "#3d3d3d" "black"))
        (zinc-800    '("#27272a" "#262626" "black"))
        ...
        (neutral-900 '("#171717" "#171717" "black"))
        ...
        (dyellow     '("#d08700" "#cd8500" "yellow"))
        (yellow-bg   '("#894b00" "#894b00" "brightblack"))
        (lime        '("#9ae600" "#9acd32" "green"))
        (bgreen      '("#7bf1a8" "#98ee90" "brightgreen"))
        (green       '("#00c950" "#32cd32" "green"))
        (dgreen      '("#008236" "#2e8b57" "green"))
        (emerald     '("#00bc7d" "#29b2aa" "green"))
        (teal        '("#00d5be" "#48d1cc" "cyan"))
        ...
        (-modeline-pad
            (when doom-windy-dark-padded-modeline
                (if (integerp doom-windy-dark-padded-modeline) doom-windy-dark-padded-modeline 4))))
    ...
    (
        ...
        (highlight
            ...
            :background (doom-blend bg dark-blue 0.4)
        ...
        (mode-line-inactive
            ...
            :underline nil)
        (mode-line-emphasis
            :foreground (if doom-windy-dark-brighter base8 builtin))
        ...
        ;;;; Package faces
        ;;;; adict
        (adict-conditional-text-face :background dsky)
        ;;;; all-the-icons
        ...
        (doom-modeline-evil-insert-state   :inherit 'doom-modeline :foreground green)
        (doom-modeline-evil-motion-state   :inherit 'doom-modeline :foreground constants)
        (doom-modeline-evil-normal-state   :inherit 'doom-modeline :foreground stone)
        ...
        (doom-modeline-boon-command-state  :inherit 'doom-modeline-evil-operator-state)
        (doom-modeline-boon-insert-state   :inherit 'doom-modeline-evil-insert-state)
        (doom-modeline-boon-special-state  :inherit 'doom-modeline-evil-user-state)
        (doom-modeline-boon-off-state      :inherit 'doom-modeline-evil-normal-state)
        ;;;; doom-themes
        ...
        ;;;; log-view
        (log-view-file    :background base3)
        (log-view-message :background base2)
        ;;;; lsp
        ...
        (mu4e-flagged-face
            :foreground indigo
            :background (doom-blend bg indigo 0.85))
        ...
        (mu4e-unread-face
            :foreground fuchsia
            :background (doom-blend bg fuchsia 0.85)
            :weight 'bold)
        ;;;; nav-flash
        (nav-flash-face :inherit 'highlight :foreground base8 :weight 'bold)
        ;;;; nerd-icons
        ...
        ((outline-1 &override)
            :foreground (doom-blend fg blue    (if doom-windy-dark-brighter 0.25 0.33)))
        ((outline-2 &override)
        ...
        (proced-sess                :foreground bsky)
        (proced-session-leader-pid  :foreground dblue :underline t)
        (proced-time-colon          :foreground violet)
        (proced-uninterruptible-sleep-status-code
            :foreground rose)
        ;;;; pulse
        (pulse-highlight-face :background dyellow)
        ...
        (shr-h2   :inherit 'outline-2)
        (shr-h3   :inherit 'outline-3)
        (shr-h4   :inherit 'outline-4)
        (shr-h5   :inherit 'outline-5)
        (shr-h6   :inherit 'outline-6)
...
;;; doom-windy-dark-theme.el ends here
