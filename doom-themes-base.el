...
(defvar doom-themes-base-faces
  '((bold        :weight 'bold :foreground (if bold 'unspecified base8))
    ...
    (geiser-font-lock-repl-output :foreground fg)
    (geiser-font-lock-repl-output-success :foreground green :background (doom-blend green bg 0.2) :extend t)
    (geiser-font-lock-repl-output-warning :foreground yellow :background (doom-blend yellow bg 0.2) :extend t)
    (geiser-font-lock-repl-output-error :foreground red :background (doom-blend red bg 0.2) :extend t)
    ;; (geiser-font-lock-repl-prompt)
    ;; (geiser-font-lock-xref-header)
    ;; (geiser-font-lock-xref-link)
    ;;;; proced <built-in>
    (proced-cpu :foreground comments)
    (proced-emacs-pid :foreground functions)
    (proced-executable :foreground green)
    ;; (proced-interruptible-sleep-status-code)
    ;; (proced-mark)
    ;; (proced-marked)
    (proced-mem :foreground comments)
    ;; (proced-memory-high-usage)
    ;; (proced-memory-low-usage)
    ;; (proced-memory-medium-usage)
    ;; (proced-pgrp)
    (proced-pid :foreground keywords)
    ;; (proced-ppid)
    ;; (proced-run-status-code)
    ;; (proced-sess)
    (proced-session-leader-pid :foreground keywords :style 'underline)
    ;; (proced-sort-header)
    (proced-time-colon :foreground fg)
